...
if ($null -eq $mountJob) { Write-WorkerStatus -Level "ERROR" -Message "Mount-Job-Konfiguration fehlt in JSON."; throw "Mount job '$jobId' not found in config JSON." }
...
if (-not [string]::IsNullOrWhiteSpace($remoteFromConfig)) {
    if ($remoteFromConfig -notmatch ":") { $remoteFromConfig = "$remoteFromConfig:" }
...
if (-not [string]::IsNullOrWhiteSpace($driveFromConfig)) {
    if ($driveFromConfig -notmatch ":$") { $driveFromConfig = "$driveFromConfig:" }
...
if ($null -eq $svc) { Write-WorkerStatus -Level "ERROR" -Message "WinFsp fehlt."; throw "WinFsp launcher service not found." }
