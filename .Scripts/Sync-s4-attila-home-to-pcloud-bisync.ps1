...
$jobId = "s4"
$jobName = "Attila Home -> pCloud"
$mode = "bisync"
$source = "C:\Users\attila\"
$destination = "pcdrive:Attila"
$jobOptions = @{ "transfers" = 8; "checkers" = 32; "drive-chunk-size" = "64M"; "skip-links" = $true }
$excludes = @(
    ".bun/**", ".cache/**", ".codex/tmp/**", ".nuget/**", ".vscode/extensions/**", "**/*.partial", "/ntuser*", "/NTUSER*",
    "AppData/Local/Autodesk/**", "AppData/Local/Comms/UnistoreDB/**", "AppData/Local/docker-secrets-engine/**", "AppData/Local/Docker/**",
    "AppData/Local/ElevatedDiagnostics", "AppData/Local/ElevatedDiagnostics/**", "AppData/Local/Google/**", "AppData/Local/Microsoft/**",
    "AppData/Local/Mozilla/**", "AppData/Local/Packages/**", "AppData/Local/PowerToys/**", "AppData/Local/Programs/**", "AppData/Local/Razer/**",
    "AppData/Local/rclone/**", "AppData/Local/Spotify/**", "AppData/Local/Temp/**", "AppData/Local/uv/**", "AppData/Local/wsl/**", "AppData/LocalLow/**",
    "AppData/Roaming/BambuStudio/**", "AppData/Roaming/Code/**", "AppData/Roaming/Mozilla/**", "AppData/Roaming/npm/**", "AppData/Roaming/OpenCode/**",
    "AppData/Roaming/Spotify/**", "AppData/Roaming/Telegram Desktop/**", "AppData/Roaming/Zoom/**", "Projects/*/.git/**", "Projects/R-Agent/.logs/**"
)
...
function Write-WorkerStatus {
    ...
    ([ordered]@{ "From.ID" = $JobGuid; "From.Name" = $jobName; "Status.Level" = $Level; "Status.Message" = $Message; "Status.Timestamp" = (Get-Date -Format "yyyy-MM-ddTHH:mm:ss") } | ConvertTo-Json -Depth 4) | Out-File -FilePath $statusPath -Encoding UTF8 -Force
...
function Get-SyncMetricsFromLog {
    ...
    $metrics = [ordered]@{ Transferred = $null; Checked = $null; Deleted = $null; Affected = $null }
    if (-not (Test-Path -LiteralPath $LogFile)) { return [pscustomobject]$metrics }

    $content = Get-Content -LiteralPath $LogFile -Raw
    $mTransferred = [regex]::Match($content, "Transferred:\s*([0-9,]+)\s*/")
    if ($mTransferred.Success) { $metrics.Transferred = [int](($mTransferred.Groups[1].Value) -replace ",", "") }
    $mChecked = [regex]::Match($content, "Checks:\s*([0-9,]+)\s*/")
    if ($mChecked.Success) { $metrics.Checked = [int](($mChecked.Groups[1].Value) -replace ",", "") }
    $mDeleted = [regex]::Match($content, "Deleted:\s*([0-9,]+)")
    if ($mDeleted.Success) { $metrics.Deleted = [int](($mDeleted.Groups[1].Value) -replace ",", "") }
    if ($null -ne $metrics.Transferred -or $null -ne $metrics.Deleted) { $metrics.Affected = [int](($metrics.Transferred ?? 0) + ($metrics.Deleted ?? 0)) }
...
if ([string]::IsNullOrWhiteSpace($LogRoot)) { $repoRoot = (Resolve-Path (Join-Path $PSScriptRoot "..")).Path; $LogRoot = Join-Path $repoRoot ".logs" }
...
if ([string]::IsNullOrWhiteSpace($rcloneExe) -or -not (Test-Path -LiteralPath $rcloneExe)) { $cmd = Get-Command rclone -ErrorAction SilentlyContinue; if ($null -eq $cmd) { throw "rclone.exe not found." }; $rcloneExe = $cmd.Source }
$rcloneConf = [string]$cfg.facts.rclone.config_path
if ([string]::IsNullOrWhiteSpace($rcloneConf) -or -not (Test-Path -LiteralPath $rcloneConf)) { $defaultConf = Join-Path $env:APPDATA "rclone\rclone.conf"; if (-not (Test-Path -LiteralPath $defaultConf)) { throw "rclone.conf not found." }; $rcloneConf = $defaultConf }

$syncJob = @($cfg.facts.automation.syncs) | Where-Object { [string]$_.id -eq $jobId } | Select-Object -First 1
if ($null -eq $syncJob) { Write-WorkerStatus -Level "ERROR" -Message "Sync-Job-Konfiguration fehlt in JSON."; throw "Sync job '$jobId' not found in config JSON." }

$enabled = $true
$enabledValue = Get-ConfigPropertyValue -Object $syncJob -Name "enabled"
if ($null -ne $enabledValue) { $enabled = [bool]$enabledValue }
if (-not $enabled) { Write-WorkerStatus -Level "INFO" -Message "Job ist deaktiviert (enabled=false)."; exit 0 }
...
if ($configuredOptions.Count -gt 0) { $jobOptions = $configuredOptions }

$excludesValue = Get-ConfigPropertyValue -Object $syncJob -Name "excludes"
if ($null -ne $excludesValue) { $excludes = @($excludesValue | ForEach-Object { [string]$_ }) }
...
if ($exitCode -eq 0) {
    ...
    if ($null -ne $metrics.Affected) { $statusMessage = "Sync erfolgreich: betroffene Dateien $($metrics.Affected), übertragen $($metrics.Transferred ?? 0), gelöscht $($metrics.Deleted ?? 0), geprüft $($metrics.Checked ?? 0)." }
    Write-WorkerStatus -Level "INFO" -Message $statusMessage
}
else {
    $statusMessage = "Sync fehlgeschlagen (ExitCode $exitCode)."
    if ($null -ne $metrics.Affected) { $statusMessage = "Sync fehlgeschlagen (ExitCode $exitCode). Letzte Metrik: betroffen $($metrics.Affected), übertragen $($metrics.Transferred ?? 0), gelöscht $($metrics.Deleted ?? 0), geprüft $($metrics.Checked ?? 0)." }
    if (-not [string]::IsNullOrWhiteSpace($errorDetail)) {
        if ($errorDetail -match "Must run --resync to recover") { $statusMessage = "$statusMessage Bisync benötigt einmalig --resync." }
        else { $statusMessage = "$statusMessage Detail: $errorDetail" }
    }
